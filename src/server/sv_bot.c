...
void BotDrawDebugPolygons(void (*drawPoly)(int color, int numPoints, float *points), int value) {
	...
	if (bot_enable && bot_debug->integer) {
		...
		if (!bot_flood) bot_flood = Cvar_Get("bot_flood", "0", 0);
		
		botlib_export->BotLibVarSet("bot_flood", bot_flood->string);
...
//		Com_Printf("calling botlib_export->Test(%d, NULL, origin, viewangles)\n", parm0);
...
/* No point in cheat protecting things when testing in debug mode */
#ifdef NDEBUG
#define BOT_CHEAT CVAR_CHEAT
#else
#define BOT_CHEAT 0
#endif
...
void SV_BotInitCvars(void) {
	...
	bot_developer = Cvar_Get("bot_developer", "0", BOT_CHEAT);			//bot developer mode
	bot_debug = Cvar_Get("bot_debug", "0", BOT_CHEAT);				//enable bot debugging
	...
	Cvar_Get("bot_visualizejumppads", "0", BOT_CHEAT);	//show jumppads
	...
	Cvar_Get("bot_thinktime", "100", BOT_CHEAT);		//msec the bots thinks
	...
	Cvar_Get("bot_testsolid", "0", BOT_CHEAT);			//test for solid areas
	Cvar_Get("bot_testclusters", "0", BOT_CHEAT);		//test the AAS clusters
	Cvar_Get("bot_fastchat", "0", 0);					//fast chatting bots
	Cvar_Get("bot_nochat", "0", 0);						//disable chats
	Cvar_Get("bot_pause", "0", BOT_CHEAT);				//pause the bots thinking
	Cvar_Get("bot_report", "0", BOT_CHEAT);			//get a full report in ctf
