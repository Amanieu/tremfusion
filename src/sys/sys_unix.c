...
void Sys_ListFilteredFiles( const char *basedir, char *subdirs, char *filter, char **list, int *numfiles )
{
	...
	while ((d = readdir(fdir)) != NULL) {
		Q_snprintf(filename, sizeof(filename), "%s/%s", search, d->d_name);
		...
		if (st.st_mode & S_IFDIR) {
			if (Q_stricmp(d->d_name, ".") && Q_stricmp(d->d_name, "..")) {
				if (strlen(subdirs)) {
					Q_snprintf( newsubdirs, sizeof(newsubdirs), "%s/%s", subdirs, d->d_name);
				}
				else {
					Q_snprintf( newsubdirs, sizeof(newsubdirs), "%s", d->d_name);
				}
		...
		Q_snprintf( filename, sizeof(filename), "%s/%s", subdirs, d->d_name );
...
char **Sys_ListFiles( const char *directory, const char *extension, char *filter, int *numfiles, qboolean wantsubs )
{
	...
	while ((d = readdir(fdir)) != NULL) {
		Q_snprintf(search, sizeof(search), "%s/%s", directory, d->d_name);
		if (stat(search, &st) == -1)
			continue;
