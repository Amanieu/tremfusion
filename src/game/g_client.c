...
char *ClientConnect( int clientNum, qboolean firstTime )
{
  ...
  // TODO: Call event here: game.on_player_connect
...
void ClientBegin( int clientNum )
{
  ...
  // TODO: Call event here: player.on_init
  ...
  // TODO: Call event here: game.on_player_begin
}
...
static int setSpawnType(scDataTypeValue_t *in, scDataTypeValue_t *out, void *closure)
{
  ...
  // Get parameters
  SC_HashGet(hash, "client", &value);
  client = G_ClientFromScript(value.data.object);
...
void ClientSpawn( gentity_t *ent, gentity_t *spawn, vec3_t origin, vec3_t angles )
{
  ...
  value.type = TYPE_OBJECT;
  value.data.object = G_GetScriptingClient(ent->client);
  SC_HashSet(hash, "client", &value);
...
void ClientDisconnect( int clientNum )
{
  ...
  // TODO: Call event here: game.on_player_disconnect
}
