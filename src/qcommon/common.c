...
void QDECL Com_Printf( const char *fmt, ... ) {
	...
	if (is_new_line && (com_timestamps && com_timestamps->integer)) {
		qtime_t realtime = {0, 0, 0, 0, 0, 0, 0 ,0 ,0};
...
void Com_Frame( void ) {
...
#else
	if ( com_speeds->integer ) {
		timeBeforeClient = timeBeforeEvents = timeAfter = Sys_Milliseconds ();
	}
#endif
	...
	if ( com_speeds->integer ) {
		...
		Com_Printf ("frame:%i msec:%3i all:%3i sv:%3i ev:%3i cl:%3i gm:%3i rf:%3i bk:%3i\n", 
					 com_frameNumber, msec, all, sv, ev, cl, time_game, time_frontend, time_backend );
	}	
