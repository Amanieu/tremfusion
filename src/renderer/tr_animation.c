...
void RB_SurfaceAnim( surfaceType_t *surf ) {
	...
	if ( tess.vertexPtr ) {
		...
		for ( j = 0; j < numVerts; j++ ) {
			...
			Vector4Copy( vertexPtr->xyz, tempVert );
			Vector4Copy( vertexPtr->normal, tempNormal );
			Vector2Copy( vertexPtr->tc1, v->texCoords );
	...
	if ( tess.indexPtr ) {
		...
		if ( tess.indexInc == sizeof(GLushort) ) {
			indexPtr = ((GLushort *)tess.indexPtr) + tess.numIndexes;
			for (j = 0 ; j < indexes ; j++) {
