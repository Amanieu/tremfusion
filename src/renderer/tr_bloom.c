...
static void inline R_Bloom_Quad( int width, int height, float texX, float texY, float texWidth, float texHeight ) {
	int x = 0;
	int y = 0;
	x = 0;
...
static void R_Bloom_InitTextures( void )
{
	...
	for (bloom.effect.width = 1;bloom.effect.width < bloom.work.width;bloom.effect.width *= 32);
	for (bloom.effect.height = 1;bloom.effect.height < bloom.work.height;bloom.effect.height *= 32);
