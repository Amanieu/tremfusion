...
static void ComputeStageIteratorFunc( void )
{
	...
	if ( glConfig.vertexBufferObjects && !r_greyscale->integer ) {
		shader.useVBO = qtrue;
	}
...
To make this quicker we do only one reverse pass of a Bubblesort.
This should move the big occluders to the front quickly, but a full
sort requires multiple frames.
...
void R_SortShaders( void ) {
	...
	tmp = tr.sortedShaders[tr.numShaders - 1];
	for ( i = tr.numShaders - 2; i >= 0; i-- ) {
		if ( tr.sortedShaders[i]->sort < tmp->sort ||
		     (tr.sortedShaders[i]->sort == tmp->sort &&
		      tr.sortedShaders[i]->QueryResult >= tmp->QueryResult) ) {
			tmp->sortedIndex = i + 1;
			tr.sortedShaders[i+1] = tmp;
			tmp = tr.sortedShaders[i];
		} else {
			tr.sortedShaders[i]->sortedIndex = i + 1;
